...
class TestStatsReporter:
    ...
    def test_collect_stats_cgroup_v2(self, mock_exists, mock_file, mock_virtual_memory, mock_cpu_percent):
        """Test stats collection using Cgroup V2."""
        mock_cpu_percent.return_value = 15.5
        
        # Mock Cgroup V2 files existing
        def exists_side_effect(path):
            return path in ["/sys/fs/cgroup/memory.current", "/sys/fs/cgroup/memory.max"]
    ...
    @patch("os.path.exists")
    def test_collect_stats_fallback(self, mock_exists, mock_process_iter, mock_virtual_memory, mock_cpu_percent):
